...
public class ConfigPanel extends HBox {
    ...
    public ConfigPanel(MainFrame mainFrame) {
        ...
        widthTextField.setOnAction(event -> {
            if (!mainFrame.controlPanel.comboBox.getSelectionModel().isEmpty()) {
                VBox vBox = (VBox) mainFrame.controlPanel.comboBox.getSelectionModel().getSelectedItem();
                for (Node n : vBox.getChildren()) {
                    if (n instanceof Rectangle) {
                        ((Rectangle) n).setWidth(Integer.valueOf(widthTextField.getText()));
                    }
                }
            }
        });

        heightTextField.setOnAction(event -> {
            if (!mainFrame.controlPanel.comboBox.getSelectionModel().isEmpty()) {
                VBox vBox = (VBox) mainFrame.controlPanel.comboBox.getSelectionModel().getSelectedItem();
                for (Node n : vBox.getChildren()) {
                    if (n instanceof Rectangle) {
                        ((Rectangle) n).setHeight(Integer.valueOf(heightTextField.getText()));
                    }
                }
            }
        });
        ...
        colorPicker.setOnAction(event -> {
            if (!mainFrame.controlPanel.comboBox.getSelectionModel().isEmpty()) {
                ...
                for (Node n : vBox.getChildren()) {
                    if (n instanceof Rectangle) {
                        ((Rectangle) n).setStroke(colorPicker.getValue());
                        ((Rectangle) n).setFill(colorPicker.getValue());
                    }
                }
            }
        });
        ...
        widthSlider.valueProperty().addListener((ObservableValue<? extends Number> observableValue, Number oldValue, Number newValue) -> {
            ...
            if (!mainFrame.controlPanel.comboBox.getSelectionModel().isEmpty()) {
                ...
                for (Node n : vBox.getChildren()) {
                    if (n instanceof Rectangle) {
                        ((Rectangle) n).setWidth(Integer.valueOf(widthTextField.getText()) / vBox.getWidth()*10);
                    }
                }
            }
        });
        ...
        heightSlider.valueProperty().addListener((ObservableValue<? extends Number> observableValue, Number oldValue, Number newValue) -> {
            ...
            if (!mainFrame.controlPanel.comboBox.getSelectionModel().isEmpty()) {
                ...
                for (Node n : vBox.getChildren()) {
                    if (n instanceof Rectangle) {
                        ((Rectangle) n).setHeight(Integer.valueOf(heightTextField.getText()) / vBox.getWidth()*10);
                    }
                }
            }
        });
